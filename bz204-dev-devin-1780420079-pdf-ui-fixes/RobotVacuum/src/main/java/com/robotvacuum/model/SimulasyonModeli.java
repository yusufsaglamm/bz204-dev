...
public class SimulasyonModeli {
    ...
    private void rastgeleIlerle() {
        ...
        // Eğer gidilmemiş (yeni) bir yol varsa, kesin olarak o yönü seçerek daha akıllı gezmesini sağlıyoruz.
        if (!gidilmemisYonler.isEmpty()) {
            secilen = gidilmemisYonler.get(rastgele.nextInt(gidilmemisYonler.size()));
        } else {
            // Eğer her yer gezilmişse normal rastgele davranır
        ...
        if (ilerleyebilirMi(spiralYonu) && spiralAdimi < spiralDogruAdimSayisi) {
            ...
            if (!ilerleyebilirMi(spiralYonu)) {
                ...
                if (!kullanilabilir.isEmpty()) {
                    // Tercihen gidilmemiş bir yön seçerek sıkışmayı engelle
                    List<Yon> gidilmemis = new ArrayList<>();
                    for (Yon y : kullanilabilir) {
                        if (!oda.getHucre(robot.getX() + y.getDx(), robot.getY() + y.getDy()).temizlendiMi()) {
                            gidilmemis.add(y);
                        }
                    }
                    if (!gidilmemis.isEmpty()) {
                        spiralYonu = gidilmemis.get(rastgele.nextInt(gidilmemis.size()));
                    } else {
